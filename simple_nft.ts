...
(async () => {
  ...
  console.log(`Shanni: ${alice.address()}`);
  ...
  console.log(`Shanni: ${await coinClient.checkBalance(alice)}`);
  console.log(`Bob: ${await coinClient.checkBalance(bob)}`);
  console.log("");

  console.log("=== Creating Collection and Token ===");

  const collectionName = "Shanni's Collection";
  const tokenName = "Artsy Token";
  const tokenPropertyVersion = 0;

  const createrAddress = alice.address().hex();
  ...
  console.log(`Shanni's collection: ${JSON.stringify(collectionData, null, 4)}`); // <:!:section_6
  ...
  console.log(`Shanni's token balance: ${aliceBalance1["amount"]}`); // <:!:section_7

  // Get the token data.
  // :!:>section_8
  const tokenData = await tokenClient.getTokenData(alice.address(), collectionName, tokenName);
  console.log(`Shanni's token data: ${JSON.stringify(tokenData, null, 4)}`); // <:!:section_8
  ...
  console.log(`Shanni's token balance: ${aliceBalance2["amount"]}`);
  ...
  console.log(`Shanni's token balance: ${aliceBalance3["amount"]}`);
  ...
  console.log(`Shanni current token ownership: ${aliceNfts.current_token_ownerships[0].amount}. Should be 1`);
